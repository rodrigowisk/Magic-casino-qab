...
// --- HELPER DE VISUALIZAÇÃO (TIPO 'ANY' PARA EVITAR ERRO DE BUILD) ---
const mapSportVisuals = (key: any) => {
    // Força conversão para string para calar o TypeScript
    ...
    if (k.includes('hockey') || k.includes('icehockey')) return { name: 'Hóquei', color: 'from-blue-400 to-blue-600', icon: '🏒' };
    if (k.includes('american') || k.includes('football')) return { name: 'Fut. Americano', color: 'from-amber-700 to-amber-900', icon: '🏈' };
    if (k.includes('cricket')) return { name: 'Cricket', color: 'from-slate-600 to-slate-800', icon: '🏏' };
    if (k.includes('rugby')) return { name: 'Rugby', color: 'from-emerald-600 to-teal-800', icon: '🏉' };
...
// --- FUNÇÃO DE NORMALIZAÇÃO (RETORNA ANY PARA EVITAR CONFLITO) ---
const getSportCategory = (game: any): any => {
    const raw = game.sportKey || game.SportKey || game.sport_key || game.SportKeyRaw || '';
    const cleanRaw = String(raw).toLowerCase();

    if (cleanRaw.includes('soccer') || cleanRaw.includes('futebol')) return 'soccer';
    if (cleanRaw.includes('basket')) return 'basketball';
    if (cleanRaw.includes('tennis')) return 'tennis';
    if (cleanRaw.includes('hockey')) return 'icehockey';
    if (cleanRaw.includes('mma') || cleanRaw.includes('ufc')) return 'mma';
    if (cleanRaw.includes('american')) return 'americanfootball';
    if (cleanRaw.includes('baseball')) return 'baseball';
    
    if (cleanRaw.includes('_')) return cleanRaw.split('_')[0];
...
const menuOptions = computed(() => {
    ...
    events.value.forEach(e => {
        // ✅ MARRETA: Força String() explicita, impossível ser undefined
    ...
    const options = Object.entries(groups).map(([key, count]) => {
        // ✅ MARRETA: Garante string antes de passar para o visual
...
const filteredEvents = computed(() => {
    ...
    // ✅ MARRETA: Força string na comparação
...
const isMarketSuspended = (odd: number) => {
    return !odd || odd <= 1.01;
};
...
const getFlagUrl = (leagueName: string) => {
  ...
  const map: Record<string, string> = { brazil: 'br', england: 'gb', spain: 'es', italy: 'it', germany: 'de', france: 'fr', usa: 'us', australia: 'au', sweden: 'se', portugal: 'pt' };
...
<template>
  <div class="space-y-4 pb-20">
    ...
    <div class="space-y-4 mt-2">
      ...
      <div v-else-if="Object.keys(groupedEvents).length === 0 && !loading" class="py-8 text-center text-stake-text">
        Nenhum jogo deste esporte ao vivo.
      </div>
      ...
      <div v-else v-for="(games, league) in groupedEvents" :key="league" class="rounded overflow-hidden">
        ...
        <div v-show="openLeagues.has(String(league))" class="bg-stake-dark border-x border-b border-stake-card/30">
          <div v-for="game in games" :key="game.gameId" class="p-4 border-b border-stake-card/30 flex flex-col md:flex-row items-center gap-4 transition-all relative group hover:-translate-y-[1px] hover:bg-[#B6FF00]/[0.12] hover:border-l-4 hover:border-l-[#B6FF00]/80 hover:shadow-[0_0_18px_rgba(182,255,0,0.25)]">
            ...
            <div class="flex-1 w-full text-white">
              <div class="flex items-center justify-between mb-2">
                ...
                <div class="flex items-center gap-2">
                    <span v-if="game.sportKey?.includes('tennis')" class="text-[10px] text-stake-text font-mono bg-black/40 px-1 rounded">Set 1</span>
                    
                    <span :class="[
                        'font-mono font-bold text-base px-2.5 py-0.5 rounded min-w-[2rem] text-center transition-colors bg-black/20',
                        game.homeScore > 0 ? 'text-yellow-400' : 'text-stake-blue'
                    ]">
                        {{ game.homeScore }}
                    </span>
                </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span :class="[
                    'font-mono font-bold text-base px-2.5 py-0.5 rounded min-w-[2rem] text-center transition-colors bg-black/20',
                    game.awayScore > 0 ? 'text-yellow-400' : 'text-stake-blue'
                ]">
                    {{ game.awayScore }}
                </span>
              </div>
                ...
                <div class="flex items-center gap-1 justify-center min-h-[20px]">
                    ...
                    <template v-else>
                        ...
                        <span :class="[
                            ...
                            isSelected(game, type as BetType) ? 'text-white' : 'text-white group-hover:scale-110',
